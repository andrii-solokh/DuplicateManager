...
export default class DuplicateMergeModal extends NavigationMixin(LightningElement) {
    @api duplicateSetId;
    ...
    async loadComparison() {
        ...
        try {
            const result = await getRecordComparison({ duplicateSetId: this.duplicateSetId });
            ...
            this.fields = result.fields.map(field => ({
                ...
                values: field.values.map(value => ({
                    ...
                    cellClass: this.getCellClass(value, field, value.recordId === this.masterRecordId)
                }))
            }));
    ...
    get filteredFields() {
        return this.fields.filter(field => {
            ...
            if (this.searchTerm) {
                ...
                const valueMatch = field.values.some(v => 
                    v.displayValue && v.displayValue.toLowerCase().includes(term)
                );
            ...
            values: field.values.map(value => ({
                ...
                cellClass: this.getCellClass(value, field, this.fieldSelections[field.apiName] === value.recordId)
            }))
        }));
    }
    ...
    handleMasterSelect(event) {
        ...
        this.records = this.records.map(record => ({
            ...
            cardClass: record.recordId === recordId ? 'master-card selected' : 'master-card'
        }));
    ...
    async handleMerge() {
        ...
        try {
            ...
            for (const [fieldName, recordId] of Object.entries(this.fieldSelections)) {

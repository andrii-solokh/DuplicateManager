...
export default class DuplicateViewer extends LightningElement {
    ...
    get jobStatusClass() {
        if (!this.currentJob) return '';
        if (this.currentJob.isComplete && this.currentJob.isSuccess) return 'job-success';
        if (this.currentJob.isComplete && !this.currentJob.isSuccess) return 'job-failed';
        return 'job-running';
    }
    ...
    async handleRunScan() {
        if (this.selectedObjectType === 'All') {
            this.showToast('Info', 'Please select a specific object type to scan.', 'info');
            return;
        }
    ...
    handleSetClick(event) {
        ...
        if (event.target.closest('.delete-btn') || event.target.closest('.merge-btn')) {
            return;
        }
